...
| --- | --- | --- |
| Framework | Leptos 0.7+ (islands) | SSR with surgical WASM hydration |
...
3. In CI, the server runs locally and all routes are pre-rendered to static HTML via `wget`
...
CI pre-render:
   ...
   +- Pre-render all known routes via wget -> static HTML
...
Client-side fuzzy search over pre-built search index. `Cmd+K` / `Ctrl+K` keyboard shortcut to open search overlay. Searches titles, headings, and content excerpts. Results ranked by relevance. Implemented as an `#[island]` component.
...
CSS-first using `animation-timeline: scroll()` and `animation-range`. Fallback: `IntersectionObserver` via `#[island]` for browsers that don't support scroll-timeline. Motion style: fade-up and slide-in, smooth and organic. Respects `prefers-reduced-motion` — all animations disabled when set.
...
`mise.toml` with tasks: `dev` (cargo leptos watch), `build` (cargo leptos build --release), `content` (validate markdown frontmatter and links).
...
All routes pre-rendered at build time. Route list derived from doc tree (finite, enumerable) plus landing page. `404.html` generated for GitHub Pages SPA fallback routing.
...
| --- | --- | --- | --- |
| 1 | v1.0 | Planning | Landing page, docs wiki, design system, deployment pipeline |
