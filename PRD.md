...
| `wasm-bindgen` + `web-sys` | Browser API access (localStorage, IntersectionObserver, Clipboard) |
...
All routes pre-rendered at build time. Route list auto-generated by `build.rs` into a `routes.txt` manifest (doc slugs from content pipeline + landing page). CI script starts the Axum server locally, iterates the manifest, and `wget`s each route to `path/index.html` (e.g., `docs/getting-started/index.html`). This ensures direct URL access works on GitHub Pages without server-side routing. `404.html` generated for unmatched routes.
